...
const uid = () => crypto.randomUUID?.() || Math.random().toString(36).slice(2, 10) + Date.now().toString(36);
const gl = s => LVL.find(l => s >= l.min && s <= l.max) || LVL[0];
const td = (d = new Date()) => d.toISOString().split("T")[0];
const tw = () => { const d = new Date(); d.setDate(d.getDate() + 1); return td(d); };
...
const DB = {
  async g(k) { try { const raw = localStorage.getItem(k); return raw ? JSON.parse(raw) : null; } catch { return null; } },
...
const X = {
  ...
  ac: "#f97316", acG: "rgba(249,115,22,.1)", acS: "#fdba74", acD: "#c2410c",
...
@keyframes glow{0%,100%{box-shadow:0 0 8px ${X.ac}30}50%{box-shadow:0 0 20px ${X.ac}50}}
@keyframes shimmer{0%{background-position:-200% 0}100%{background-position:200% 0}}
.anim-fu{animation:fadeUp .35s ease both}
.anim-fi{animation:fadeIn .3s ease both}
.anim-si{animation:scaleIn .25s ease both}
.anim-su{animation:slideUp .4s ease both}
`;
...
const Ic = ({ n, s = 16, c = "currentColor" }) => {
  ...
  const d = {
    ...
    trophy: <><path d="M6 9H4.5a2.5 2.5 0 010-5H6m12 5h1.5a2.5 2.5 0 000-5H18" /><path d="M4 22h16" /><path d="M10 14.66V17c0 .55-.47.98-.97 1.21C7.85 18.75 7 20 7 22h10c0-2-0.85-3.25-2.03-3.79A1.07 1.07 0 0114 17v-2.34" /><path d="M18 2H6v7a6 6 0 0012 0V2z" /></>,
...
// ── PRIMITIVES ──
const Btn = ({ ch, v = "primary", sm, onClick, dis, sty, icon, full }) => {
  const b = { display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, border: "none",
    cursor: dis ? "not-allowed" : "pointer", fontFamily: "'Outfit'", fontWeight: 600, borderRadius: 10,
    transition: "all .15s ease", opacity: dis ? .4 : 1, whiteSpace: "nowrap", width: full ? "100%" : "auto" };
  const vs = {
  ...
  return <button style={{ ...b, ...vs[v], ...sty }} onClick={onClick} disabled={dis}>{icon && <Ic n={icon} s={sm ? 13 : 15} />}{ch}</button>;
};
...
const Modal = ({ open, onClose, title, ch, w = 460 }) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 14 }} onClick={onClose}>
      <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,.7)", backdropFilter: "blur(8px)" }} className="anim-fi" />
      <div onClick={e => e.stopPropagation()} className="anim-si"
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <h2 style={{ fontSize: 16, fontWeight: 700, color: X.t, fontFamily: "'Outfit'", margin: 0 }}>{title}</h2>
...
const Empty = ({ icon, title, sub, action }) => (
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "32px 16px", textAlign: "center" }} className="anim-fu">
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 9999, pointerEvents: "none" }}>
      {Array.from({ length: 40 }, (_, i) => (
        <div key={i} style={{
          ...
          animation: `confetti ${Math.random() * 2 + 1.5}s ease-in forwards`,
...
function Toast({ msg, type = "success", onDone }) {
  ...
  const cc = { success: X.g, error: X.r, info: X.ac, badge: X.y };
  const icons = { success: "✓", error: "!", info: "ℹ", badge: "🏅" };
  return (
    <div style={{ position: "fixed", top: 14, left: "50%", transform: "translateX(-50%)", zIndex: 9998, background: X.s2,
...
// ── TASK FORM ──
function TaskForm({ open, onClose, onSave, task, projects }) {
  const blank = { title: "", description: "", category: "brain_out", urgency: 3, importance: 3, tediousness: 1, effort_minutes: 30, project_id: "" };
  const [f, sF] = useState(blank);
  useEffect(() => {
    if (open) sF(task ? { title: task.title || "", description: task.description || "", category: task.category || "brain_out",
      urgency: task.urgency || 3, importance: task.importance || 3, tediousness: task.tediousness || 1,
      effort_minutes: task.effort_minutes || 30, project_id: task.project_id || "" } : blank);
  }, [task, open]);

  return (
    <Modal open={open} onClose={onClose} title={task ? "Editar Tarefa" : "Nova Tarefa"} ch={
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
            {CATS.map(c => (
              <button key={c.id} onClick={() => sF(p => ({ ...p, category: c.id }))}
                style={{ display: "flex", alignItems: "center", gap: 4, padding: "5px 10px", borderRadius: 8,
                  ...
                  color: f.category === c.id ? c.color : X.t3, fontSize: 11, fontFamily: "'Outfit'", fontWeight: 500, cursor: "pointer", transition: "all .12s" }}>
                {c.icon} {c.label}
              </button>
            ))}
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          ...
          <Sel label="Projeto" value={f.project_id} onChange={v => sF(p => ({ ...p, project_id: v }))}
            opts={[{ v: "", l: "Sem projeto" }, ...projects.map(p => ({ v: p.id, l: p.name }))]} />
        </div>
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 4 }}>
          <Btn v="secondary" onClick={onClose} ch="Cancelar" />
          <Btn onClick={() => { if (f.title.trim()) { onSave({ ...f, title: f.title.trim() }); onClose(); } }} dis={!f.title.trim()} ch={task ? "Salvar" : "Criar tarefa"} />
        </div>
      </div>
    } />
  );
...
function ProjForm({ open, onClose, onSave, proj }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={proj ? "Editar Projeto" : "Novo Projeto"} w={350} ch={
      <div>
        <Inp label="Nome" value={n} onChange={sN} ph="Ex: Trabalho, Estudos..." sty={{ marginBottom: 12 }} />
        ...
        <div style={{ display: "flex", gap: 7, marginBottom: 18 }}>
          {cs.map(x => <div key={x} onClick={() => sC(x)} style={{ width: 24, height: 24, borderRadius: "50%", background: x, cursor: "pointer",
            border: c === x ? "3px solid #fff" : "3px solid transparent", boxShadow: c === x ? `0 0 0 2px ${x}` : "none", transition: "all .12s" }} />)}
        </div>
...
function TC({ task, projects, onEdit, onDelete, onStatus, compact, isFrog, showFrog, onFrog }) {
  ...
  return (
    <div className="anim-fu" onMouseEnter={() => sH(true)} onMouseLeave={() => sH(false)}
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 7 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: done ? X.t3 : X.t, fontFamily: "'Outfit'",
            textDecoration: done ? "line-through" : "none", marginBottom: 3, lineHeight: 1.3 }}>
            {isFrog && "🐸 "}{task.title}
          </div>
        ...
        <div style={{ display: "flex", gap: 1, flexShrink: 0, opacity: hov || compact ? 1 : .5, transition: "opacity .15s" }}>
...
function PlanModal({ open, onClose, tasks, projects, plans, onSave, targetDate }) {
  const [sel, sSel] = useState([]); const [frog, sFrog] = useState(null); const [step, sStep] = useState(1); const [dragI, sDragI] = useState(null);
  ...
  useEffect(() => { if (open) { sSel([]); sFrog(null); sStep(1); } }, [open]);
  ...
  const save = () => {
    const pt = sel.map((id, i) => ({ id: uid(), task_id: id, order_index: i, is_frog: id === frog, status: "not_started", was_completed: false }));
    onSave({ id: uid(), date: targetDate, status: "planned", planned_at: new Date().toISOString(), plan_tasks: pt, day_score: 0, bonus_score: 0, total_score: 0, had_exercise: false, completed_frog: false, completed_all_categories: false });
    onClose();
  };
  ...
  return (
    <Modal open={open} onClose={onClose} title={`Planejar ${fd(targetDate)}`} ch={
      <div>
        {/* Steps */}
        <div style={{ display: "flex", gap: 2, marginBottom: 14 }}>
          {["Selecionar", "Ordenar & 🐸", "Confirmar"].map((s, i) => (
            <div key={i} style={{ flex: 1, textAlign: "center", padding: "5px 0", borderRadius: 6, fontSize: 10, fontWeight: 700, fontFamily: "'Outfit'",
              background: step === i + 1 ? X.acG : "transparent", color: step === i + 1 ? X.ac : X.t3, transition: "all .15s",
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            ...
            <div style={{ display: "flex", gap: 2 }}>
              {CATS.map(c => <div key={c.id} style={{ width: 18, height: 18, borderRadius: 4, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9,
                background: cc.has(c.id) ? `${c.color}20` : X.bg, border: `1px solid ${cc.has(c.id) ? c.color + "40" : X.brd}`, opacity: cc.has(c.id) ? 1 : .3, transition: "all .2s" }}>{c.icon}</div>)}
            </div>
          </div>
          {sugg.length > 0 && <>
            <div style={{ fontSize: 10, fontWeight: 700, color: X.ac, fontFamily: "'Outfit'", marginBottom: 4, display: "flex", alignItems: "center", gap: 4 }}>💡 Sugestões inteligentes</div>
        ...
        {step === 2 && <>
          <p style={{ fontSize: 11, color: X.ac, fontFamily: "'Outfit'", margin: "0 0 10px", lineHeight: 1.4 }}>
            Arraste para reordenar · Toque 🐸 para marcar o sapo do dia <span style={{ color: X.t3 }}>(+0.3 pts)</span>
          </p>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {selT.map((t, i) => (
              <div key={t.id} draggable onDragStart={() => sDragI(i)} onDragOver={e => e.preventDefault()} onDrop={() => drop(i)}
                style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 10px", borderRadius: 9, background: X.card,
                  border: `1px solid ${frog === t.id ? X.y + "45" : X.brd}`, cursor: "grab", transition: "border .12s" }}>
                <span style={{ color: X.t3, display: "flex" }}><Ic n="grip" s={14} c={X.t3} /></span>
                <span style={{ fontSize: 10, color: X.t3, fontFamily: "'JetBrains Mono'", width: 16, textAlign: "center" }}>{i + 1}</span>
                ...
                <button onClick={() => sFrog(frog === t.id ? null : t.id)}
                  style={{ background: frog === t.id ? `${X.y}18` : "none", border: `1.5px solid ${frog === t.id ? X.y : X.brd}`,
                    borderRadius: 6, padding: "2px 6px", cursor: "pointer", fontSize: 13, transition: "all .12s" }}>🐸</button>
              </div>
            ))}
          </div>
        ...
        {step === 3 && <>
          <div style={{ background: X.bg, borderRadius: 12, padding: 14, marginBottom: 12, border: `1px solid ${X.brd}` }}>
            ...
            <div style={{ display: "flex", gap: 3, marginTop: 8 }}>
              {CATS.map(c => <div key={c.id} style={{ width: 20, height: 20, borderRadius: 5, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10,
                background: cc.has(c.id) ? `${c.color}18` : X.s1, border: `1px solid ${cc.has(c.id) ? c.color + "35" : X.brd}`, opacity: cc.has(c.id) ? 1 : .3 }}>{c.icon}</div>)}
            </div>
          </div>
          {cc.size === 6 && <div style={{ background: `${X.g}08`, border: `1px solid ${X.g}20`, borderRadius: 8, padding: "7px 12px", marginBottom: 10, fontSize: 11, color: X.g, fontFamily: "'Outfit'", fontWeight: 600 }}>
            ⭐ 6/6 categorias! Complete tudo → +1.0 bônus</div>}
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <Btn v="ghost" onClick={() => sStep(2)} ch="← Voltar" sm />
            <Btn onClick={save} icon="check" ch="Salvar plano" sm />
          </div>
        </>}
      </div>
    } />
...
function CloseModal({ open, onClose, plan, tasks, onClosePlan }) {
  ...
  const pend = pts.filter(t => t.status === "not_started"); const allD = pend.every(t => dec[t.task_id]);
  const sc = calcScore(plan.plan_tasks || [], tasks);

  return (
    <Modal open={open} onClose={onClose} title="Fechar o dia" w={440} ch={
      <div>
        {pend.length > 0 && <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {pend.map(t => (
              <div key={t.task_id} style={{ background: X.bg, borderRadius: 8, padding: 8, border: `1px solid ${X.brd}` }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: X.t, fontFamily: "'Outfit'", marginBottom: 5, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.title}</div>
                <div style={{ display: "flex", gap: 4 }}>
                  {[{ id: "tomorrow", l: "→ Amanhã", c: X.y }, { id: "return", l: "↩ Backlog", c: X.p }].map(o => (
                    <button key={o.id} onClick={() => sDec(p => ({ ...p, [t.task_id]: o.id }))}
                      style={{ flex: 1, padding: "4px 6px", borderRadius: 6, fontSize: 10, fontWeight: 600, fontFamily: "'Outfit'", cursor: "pointer",
                        ...
                        color: dec[t.task_id] === o.id ? o.c : X.t3, transition: "all .1s" }}>{o.l}</button>
                  ))}
                </div>
              </div>
            ))}
        ...
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 12 }}>
          <ScoreRing value={sc.ts} max={2} size={90} label="pontos" />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 5, marginBottom: 12 }}>
          {[{ ok: sc.ex, i: "🏃", l: "Exercício", p: .3 }, { ok: sc.fr, i: "🐸", l: "Sapo", p: .3 },
            ...
            <div key={s.l} style={{ padding: 6, borderRadius: 7, background: X.bg, textAlign: "center" }}>
              <div style={{ fontSize: 14 }}>{s.ok ? "✅" : s.i}</div>
              <div style={{ fontSize: 9, color: X.t3 }}>{s.l}</div>
              <div style={{ fontSize: 10, fontWeight: 700, color: s.ok ? X.g : X.t3, fontFamily: "'JetBrains Mono'" }}>+{s.p.toFixed(1)}</div>
            </div>
          ))}
        </div>
        {sc.ac && <div style={{ textAlign: "center", marginBottom: 10, padding: "5px 0", background: `${X.g}08`, borderRadius: 6, fontSize: 11, color: X.g, fontWeight: 700 }}>⭐ Bônus categorias: +1.0</div>}

        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          <Btn v="secondary" onClick={onClose} ch="Cancelar" sm />
          <Btn v="success" onClick={() => { onClosePlan(plan.id, sc, dec); onClose(); }} dis={pend.length > 0 && !allD} icon="lock" ch="Fechar dia" sm />
        </div>
      </div>
    } />
  );
...
// ── TAB: HOJE ──
function TabToday({ user, tasks, projects, plans, sT, sPl, sU, tst, sCf }) {
  const today = td(); const tw_ = tw(); const tp = plans.find(p => p.date === today);
  const [showPlan, sShowPlan] = useState(false); const [showClose, sShowClose] = useState(false); const [target, sTarget] = useState(tw_);
  const lv = gl(user.score);

  const planT = useMemo(() => {
    if (!tp) return [];
    return (tp.plan_tasks || []).map(pt => { const t = tasks.find(x => x.id === pt.task_id); return t ? { ...t, is_frog: pt.is_frog, order_index: pt.order_index } : null; }).filter(Boolean).sort((a, b) => a.order_index - b.order_index);
  }, [tp, tasks]);
  ...
  const hStatus = (id, s) => {
    sT(tasks.map(t => t.id === id ? { ...t, status: s, completed_at: s === "done" ? new Date().toISOString() : null } : t));
    if (tp) sPl(plans.map(p => p.id === tp.id ? { ...p, plan_tasks: p.plan_tasks.map(pt => pt.task_id === id ? { ...pt, status: s, was_completed: s === "done" } : pt) } : p));
    if (s === "done") tst("Tarefa concluída! ✨");
  };

  const hSavePlan = plan => {
    const ids = plan.plan_tasks.map(pt => pt.task_id);
    sT(tasks.map(t => ids.includes(t.id) ? { ...t, planned_date: plan.date, status: "not_started" } : t));
    sPl([...plans.filter(p => p.date !== plan.date), plan]);
    tst(`Plano para ${fd(plan.date)} salvo!`);
  };

  const hClose = (pid, sd, dec) => {
    let ut = [...tasks]; const nd = tw();
    Object.entries(dec).forEach(([id, d]) => {
      ut = ut.map(t => { if (t.id === id) { if (d === "tomorrow") return { ...t, status: "not_started", planned_date: nd }; if (d === "return") return { ...t, status: "not_started", planned_date: null }; } return t; });
    });
    sT(ut);
    sPl(plans.map(p => p.id === pid ? { ...tp, ...sd, status: "closed", closed_at: new Date().toISOString(), had_exercise: sd.ex, completed_frog: sd.fr, completed_all_categories: sd.ac, day_score: sd.ds, bonus_score: sd.bs, total_score: sd.ts } : p));
    sU({ ...user, score: rnd(user.score + sd.ts) });
  ...
  return (
    <div style={{ padding: "12px 12px 96px" }}>
      {/* Header */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }} className="anim-fu">
        <div>
          <h2 style={{ fontFamily: "'Outfit'", fontSize: 19, fontWeight: 800, color: X.t, margin: 0 }}>Olá, {user.name}</h2>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 16, fontWeight: 700, color: lv.c }}>{user.score.toFixed(1)}</div>
            <div style={{ fontFamily: "'Outfit'", fontSize: 9, color: X.t3, display: "flex", alignItems: "center", gap: 2, justifyContent: "flex-end" }}>{lv.i} {lv.n}</div>
          </div>
        </div>
      </div>
      ...
      {/* Score card */}
      {sc && tp?.status !== "closed" && (
        <div className="anim-su" style={{ background: `linear-gradient(145deg, ${X.s1}, ${X.s2})`, borderRadius: 16, border: `1px solid ${X.brd}`, padding: 16, marginBottom: 12 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 5 }}>
                {[{ ok: sc.ex, i: "🏃", l: "Exercício" }, { ok: sc.fr, i: "🐸", l: "Sapo" }, { ok: sc.dn === sc.tt && sc.tt > 0, i: "📋", l: `${sc.dn}/${sc.tt}` }].map(s => (
                  <div key={s.l} style={{ textAlign: "center", padding: "4px 2px", borderRadius: 6, background: s.ok ? `${X.g}10` : X.bg }}>
                    <div style={{ fontSize: 12 }}>{s.ok ? "✅" : s.i}</div>
                    <div style={{ fontSize: 8, color: X.t3, fontFamily: "'Outfit'" }}>{s.l}</div>
                  </div>
                ))}
              </div>
              <div style={{ display: "flex", gap: 2, marginTop: 6 }}>
                {CATS.map(c => <div key={c.id} style={{ flex: 1, height: 4, borderRadius: 2, background: cc.has(c.id) ? c.color : X.brd, transition: "background .3s", opacity: cc.has(c.id) ? 1 : .3 }} />)}
              </div>
              {cc.size > 0 && <div style={{ fontSize: 8, color: X.t3, fontFamily: "'Outfit'", marginTop: 2, textAlign: "right" }}>{cc.size}/6 categorias</div>}
            </div>
          </div>
        </div>
      )}
      ...
      {/* Closed */}
      {tp?.status === "closed" && (
        <div className="anim-su" style={{ background: `linear-gradient(145deg, ${X.g}06, ${X.p}06)`, borderRadius: 16, border: `1px solid ${X.g}20`, padding: 20, marginBottom: 12, textAlign: "center" }}>
          <div style={{ fontSize: 36, marginBottom: 4 }}>🏆</div>
          <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 26, fontWeight: 700, color: X.g }}>+{tp.total_score?.toFixed(1)}</div>
      ...
      {/* CTAs */}
      <div style={{ display: "flex", gap: 6, marginBottom: 14 }} className="anim-fu">
        {!tp && <Btn full onClick={() => { sTarget(today); sShowPlan(true); }} icon="sun" ch="Planejar hoje" />}
        {tp && tp.status !== "closed" && <Btn v="success" full onClick={() => sShowClose(true)} icon="lock" ch="Fechar dia" />}
      ...
      {planT.length > 0 && tp?.status !== "closed" ? <>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
          {planT.map((t, i) => <div key={t.id} style={{ animationDelay: `${i * 50}ms` }} className="anim-fu">
...
// ── TAB: TAREFAS ──
function TabTasks({ tasks, projects, sT, sP, tst }) {
  ...
  const add = f => { sT([...tasks, { ...f, id: uid(), status: "not_started", source: "manual", order_index: tasks.length, created_at: new Date().toISOString() }]); tst("Tarefa criada!"); };
  const edit = (id, f) => { sT(tasks.map(t => t.id === id ? { ...t, ...f } : t)); tst("Atualizada!"); };
  const del = id => { if (confirm("Excluir?")) sT(tasks.filter(t => t.id !== id)); };
  const stat = (id, s) => sT(tasks.map(t => t.id === id ? { ...t, status: s, completed_at: s === "done" ? new Date().toISOString() : null } : t));

  return (
    <div style={{ padding: "12px 12px 96px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }} className="anim-fu">
        <h2 style={{ fontFamily: "'Outfit'", fontSize: 19, fontWeight: 800, color: X.t, margin: 0 }}>Tarefas</h2>
        <div style={{ display: "flex", gap: 4 }}>
      ...
      <div style={{ display: "flex", gap: 2, marginBottom: 12, background: X.bg, borderRadius: 8, padding: 2 }}>
        {[{ id: "tasks", l: "Backlog" }, { id: "projects", l: "Projetos" }].map(v => (
          <button key={v.id} onClick={() => sVw(v.id)} style={{ flex: 1, padding: "7px 0", borderRadius: 6, border: "none", cursor: "pointer", fontFamily: "'Outfit'", fontSize: 12, fontWeight: 600,
            background: vw === v.id ? X.s2 : "transparent", color: vw === v.id ? X.t : X.t3, transition: "all .12s" }}>{v.l}</button>
        ))}
      </div>

      {vw === "tasks" ? <>
        {sF && <div style={{ background: X.s1, borderRadius: 9, border: `1px solid ${X.brd}`, padding: 10, marginBottom: 10, display: "flex", flexDirection: "column", gap: 6 }} className="anim-si">
        ...
        {!fil.length ? <Empty icon="✨" title="Nenhuma tarefa" sub="Crie tarefas para seus planos" action={<Btn sm onClick={() => { sET(null); sSTF(true); }} icon="plus" ch="Criar" />} />
          : <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
            {fil.map((t, i) => <div key={t.id} style={{ animationDelay: `${i * 30}ms` }} className="anim-fu">
              <TC task={t} projects={projects} onEdit={t => { sET(t); sSTF(true); }} onDelete={del} onStatus={stat} />
            </div>)}
            <div style={{ textAlign: "center", marginTop: 3 }}><span style={{ fontFamily: "'JetBrains Mono'", fontSize: 9, color: X.t3 }}>{fil.length} tarefa{fil.length !== 1 && "s"}</span></div>
          </div>}
      </> : <>
        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 8 }}>
          <Btn sm onClick={() => { sEP(null); sSPF(true); }} icon="plus" ch="Novo" />
        </div>
        {!projects.length ? <Empty icon="📁" title="Nenhum projeto" sub="Organize por contexto" action={<Btn sm onClick={() => { sEP(null); sSPF(true); }} icon="plus" ch="Criar" />} />
          : <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
            {projects.map((p, i) => { const pt = tasks.filter(t => t.project_id === p.id && !t.archived_at); const pd = pt.filter(t => t.status === "done").length; const pct = pt.length ? pd / pt.length : 0;
              return (<div key={p.id} className="anim-fu" style={{ animationDelay: `${i * 40}ms`, background: X.card, borderRadius: 10, border: `1px solid ${X.brd}`, padding: "11px 13px", borderLeft: `3px solid ${p.color}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div><div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 3 }}>
                    <div style={{ width: 8, height: 8, borderRadius: "50%", background: p.color }} /><span style={{ fontFamily: "'Outfit'", fontSize: 13, fontWeight: 600, color: X.t }}>{p.name}</span></div>
                    <span style={{ fontFamily: "'Outfit'", fontSize: 10, color: X.t2 }}>{pd}/{pt.length} concluídas</span>
                  </div>
                  <div style={{ display: "flex", gap: 2 }}>
                    ...
                    <button onClick={() => { if (confirm("Excluir?")) { sP(projects.filter(x => x.id !== p.id)); sT(tasks.map(t => t.project_id === p.id ? { ...t, project_id: "" } : t)); } }}
                      style={{ background: "none", border: "none", color: X.t3, cursor: "pointer", padding: 4 }}><Ic n="trash" s={12} /></button>
                  </div>
                </div>
                <div style={{ height: 3, background: X.brd, borderRadius: 2, marginTop: 6, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${pct * 100}%`, background: p.color, borderRadius: 2, transition: "width .3s" }} />
                </div>
              </div>); })}
          </div>}
      </>}

      <TaskForm open={sTF} onClose={() => { sSTF(false); sET(null); }} onSave={f => eT ? edit(eT.id, f) : add(f)} task={eT} projects={projects} />
      <ProjForm open={sPF} onClose={() => { sSPF(false); sEP(null); }} onSave={f => {
        if (eP) sP(projects.map(p => p.id === eP.id ? { ...p, ...f } : p));
        else sP([...projects, { ...f, id: uid(), created_at: new Date().toISOString() }]);
      }} proj={eP} />
    </div>
  );
}
...
// ── TAB: MISSÃO ──
function TabMission({ user, tasks, plans }) {
  const lv = gl(user.score); const nl = LVL.find(l => l.min > user.score);
  const lp = nl ? (user.score - lv.min) / (nl.min - lv.min) : 1;
  ...
  for (let i = 0; i < 365; i++) { const d = new Date(tdy); d.setDate(d.getDate() - i); if (plans.some(p => p.date === td(d) && (p.status === "planned" || p.status === "closed"))) streak++; else break; }
  ...
  const avg = closed.length ? closed.reduce((s, p) => s + (p.total_score || 0), 0) / closed.length : 0;
  ...
  const badges = [
    ...
    { c: "first_frog", n: "Primeiro Sapo", i: "🐸", d: "Concluir o primeiro sapo", ok: frogs >= 1 },
  ...
  const cal = Array.from({ length: 35 }, (_, i) => { const d = new Date(); d.setDate(d.getDate() - (34 - i)); return { date: td(d), day: d.getDate(), wd: d.getDay(), plan: plans.find(p => p.date === td(d)) }; });

  return (
    <div style={{ padding: "12px 12px 96px" }}>
      <h2 style={{ fontFamily: "'Outfit'", fontSize: 19, fontWeight: 800, color: X.t, margin: "0 0 14px" }} className="anim-fu">Missão</h2>

      {/* Level card */}
      <div className="anim-su" style={{ background: `linear-gradient(145deg, ${lv.c}08, ${X.p}06)`, borderRadius: 16, border: `1px solid ${lv.c}20`, padding: 20, marginBottom: 14, textAlign: "center" }}>
        <div style={{ fontSize: 44, marginBottom: 4 }}>{lv.i}</div>
        <div style={{ fontFamily: "'Outfit'", fontSize: 11, fontWeight: 700, color: X.t3, textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 2 }}>Nível {LVL.indexOf(lv) + 1}</div>
        <div style={{ fontFamily: "'Outfit'", fontSize: 22, fontWeight: 800, color: X.t, marginBottom: 2 }}>{lv.n}</div>
        <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 28, fontWeight: 700, color: lv.c }}>{user.score.toFixed(1)}</div>
        <div style={{ fontFamily: "'Outfit'", fontSize: 10, color: X.t3, marginTop: 2, marginBottom: 8 }}>{nl ? `${rnd(nl.min - user.score, 1)} pts para ${nl.n}` : "Nível máximo!"}</div>
        {nl && <div style={{ height: 6, background: X.bg, borderRadius: 3, overflow: "hidden", maxWidth: 200, margin: "0 auto" }}>
          <div style={{ height: "100%", width: `${lp * 100}%`, background: `linear-gradient(90deg, ${lv.c}, ${X.p})`, borderRadius: 3, transition: "width .4s" }} /></div>}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 5, marginBottom: 14 }}>
        {[{ l: "Streak", v: `${streak}d`, i: "🔥", c: X.ac }, { l: "Sapos", v: frogs, i: "🐸", c: X.y }, { l: "Exercício", v: `${exDs}d`, i: "🏃", c: X.g }, { l: "Concluídas", v: totalDone, i: "✅", c: X.p }].map((s, i) => (
          <div key={s.l} className="anim-fu" style={{ animationDelay: `${i * 60}ms`, background: X.card, borderRadius: 10, border: `1px solid ${X.brd}`, padding: "8px 4px", textAlign: "center" }}>
            <div style={{ fontSize: 14 }}>{s.i}</div>
            <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 15, fontWeight: 700, color: s.c }}>{s.v}</div>
            <div style={{ fontFamily: "'Outfit'", fontSize: 8, color: X.t3 }}>{s.l}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 2 }}>
          {["D", "S", "T", "Q", "Q", "S", "S"].map((d, i) => <div key={i} style={{ textAlign: "center", fontSize: 8, color: X.t3, fontFamily: "'Outfit'", paddingBottom: 1 }}>{d}</div>)}
          ...
          {cal.map(d => {
            ...
            return <div key={d.date} title={`${d.date}: ${cl ? s.toFixed(1) : pl ? "planejado" : "—"}`}
              style={{ aspectRatio: "1", borderRadius: 3, background: bg, display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 8, color: cl ? X.t : X.t3, fontFamily: "'JetBrains Mono'", border: d.date === td() ? `1.5px solid ${X.ac}` : "none", transition: "background .2s" }}>{d.day}</div>;
          })}
        </div>
        <div style={{ display: "flex", gap: 6, justifyContent: "center", marginTop: 4 }}>
          {[{ c: `${X.r}18`, l: "Baixo" }, { c: `${X.y}22`, l: "Médio" }, { c: `${X.g}28`, l: "Bom" }, { c: `${X.g}50`, l: "Ótimo" }].map(x => (
            <div key={x.l} style={{ display: "flex", alignItems: "center", gap: 2, fontSize: 8, color: X.t3, fontFamily: "'Outfit'" }}><div style={{ width: 8, height: 8, borderRadius: 2, background: x.c }} />{x.l}</div>
          ))}
        </div>
      </div>

      {/* Badges */}
      <h3 style={{ fontFamily: "'Outfit'", fontSize: 12, fontWeight: 700, color: X.t, margin: "0 0 6px" }}>
        Badges <span style={{ fontWeight: 400, color: X.t3 }}>({badges.filter(b => b.ok).length}/{badges.length})</span>
      </h3>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5 }}>
        {badges.map((b, i) => (
          <div key={b.c} className="anim-fu" style={{ animationDelay: `${i * 40}ms`, background: b.ok ? `${X.ac}06` : X.card, borderRadius: 10,
            border: `1px solid ${b.ok ? X.ac + "25" : X.brd}`, padding: 10, opacity: b.ok ? 1 : .35, transition: "all .2s" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 2 }}>
              <span style={{ fontSize: 16 }}>{b.i}</span>
              <span style={{ fontFamily: "'Outfit'", fontSize: 11, fontWeight: 600, color: X.t }}>{b.n}</span>
            </div>
...
// ── SETTINGS ──
function Sett({ open, onClose, user, onSave, onLogout }) {
  const [n, sN] = useState(""); const [pt, sPt] = useState("21:00"); const [st, sSt] = useState("07:00");
  useEffect(() => { if (user && open) { sN(user.name); sPt(user.daily_plan_time); sSt(user.day_start_time); } }, [user, open]);
  return <Modal open={open} onClose={onClose} title="Configurações" w={350} ch={
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <Btn onClick={() => { onSave({ name: n, daily_plan_time: pt, day_start_time: st }); onClose(); }} full ch="Salvar" />
...
function Auth({ onLogin }) {
  const [n, sN] = useState(""); const [ld, sLd] = useState(true);
  useEffect(() => { (async () => { const u = await DB.g("dia-user"); if (u) onLogin(u); else sLd(false); })(); }, []);

  const go = async () => {
    if (!n.trim()) return;
    const u = { id: uid(), name: n.trim(), score: 0, day_start_time: "07:00", daily_plan_time: "21:00", created_at: new Date().toISOString() };
    await DB.s("dia-user", u); await DB.s("dia-tasks", []); await DB.s("dia-projects", []); await DB.s("dia-plans", []);
    onLogin(u);
  };

  if (ld) return <div style={{ minHeight: "100vh", background: X.bg, display: "flex", alignItems: "center", justifyContent: "center" }}><div style={{ fontSize: 48, animation: "pulse 1.5s infinite" }}>🎯</div></div>;

  return (
    <div style={{ minHeight: "100vh", background: X.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ textAlign: "center", maxWidth: 340, width: "100%" }} className="anim-su">
        <div style={{ fontSize: 52, marginBottom: 6 }}>🎯</div>
        <h1 style={{ fontFamily: "'Outfit'", fontSize: 48, fontWeight: 900, color: X.ac, margin: "0 0 2px", letterSpacing: 6, textTransform: "uppercase" }}>
          DIA
        </h1>
        <p style={{ fontFamily: "'Outfit'", fontSize: 13, color: X.t2, margin: "0 0 28px", letterSpacing: .5 }}>Planeje · Execute · Evolua</p>
        <div style={{ background: X.s1, borderRadius: 18, border: `1px solid ${X.brd}`, padding: 24 }}>
          <Inp label="Seu nome" value={n} onChange={sN} ph="Como quer ser chamado?" sty={{ marginBottom: 18, textAlign: "left" }} />
          <Btn onClick={go} dis={!n.trim()} full sty={{ padding: "12px 20px", fontSize: 15 }} ch="Começar missão 🚀" />
        </div>
        <p style={{ fontFamily: "'Outfit'", fontSize: 10, color: X.t3, marginTop: 14 }}>Dados salvos localmente entre sessões</p>
      </div>
    </div>
  );
}
...
export default function DiaApp() {
  const [user, sUser] = useState(null); const [tasks, sTasks] = useState([]); const [projects, sProjects] = useState([]); const [plans, sPlans] = useState([]);
  const [tab, sTab] = useState("today"); const [ld, sLd] = useState(true); const [sett, sSett] = useState(false); const [toast, sToast] = useState(null); const [confetti, sConfetti] = useState(false);

  useEffect(() => { if (confetti) { const t = setTimeout(() => sConfetti(false), 3000); return () => clearTimeout(t); } }, [confetti]);

  const sU = u => { sUser(u); DB.s("dia-user", u); }; const sT = t => { sTasks(t); DB.s("dia-tasks", t); };
  const sP = p => { sProjects(p); DB.s("dia-projects", p); }; const sPl = p => { sPlans(p); DB.s("dia-plans", p); };
  const tst = (m, t = "success") => sToast({ msg: m, type: t });

  const login = u => { sUser(u); sLd(false); (async () => { sTasks(await DB.g("dia-tasks") || []); sProjects(await DB.g("dia-projects") || []); sPlans(await DB.g("dia-plans") || []); })(); };

  if (!user && ld) return <Auth onLogin={login} />;
  if (!user) return <Auth onLogin={login} />;

  const tabs = [{ id: "today", i: "⚡", l: "Hoje" }, { id: "tasks", i: "📋", l: "Tarefas" }, { id: "mission", i: "🎯", l: "Missão" }];

  return (
    <div style={{ minHeight: "100vh", background: X.bg, fontFamily: "'Outfit',sans-serif", color: X.t, maxWidth: 480, margin: "0 auto", position: "relative", overflow: "hidden" }}>
      <style>{CSS}</style>
      ...
      {/* Content */}
      {tab === "today" && <TabToday user={user} tasks={tasks} projects={projects} plans={plans} sT={sT} sPl={sPl} sU={sU} tst={tst} sCf={sConfetti} />}
      {tab === "tasks" && <TabTasks tasks={tasks} projects={projects} sT={sT} sP={sP} tst={tst} />}
      {tab === "mission" && <TabMission user={user} tasks={tasks} plans={plans} />}
      ...
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 480,
        ...
        {tabs.map(t => (
          <button key={t.id} onClick={() => sTab(t.id)} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 1, padding: "6px 0",
            background: "none", border: "none", cursor: "pointer", color: tab === t.id ? X.ac : X.t3, transition: "all .12s", position: "relative" }}>
            {tab === t.id && <div style={{ position: "absolute", top: -1, width: 20, height: 2, borderRadius: 1, background: X.ac }} />}
            <span style={{ fontSize: 17, transition: "transform .15s", transform: tab === t.id ? "scale(1.15)" : "scale(1)" }}>{t.i}</span>
            <span style={{ fontSize: 9, fontFamily: "'Outfit'", fontWeight: tab === t.id ? 700 : 500 }}>{t.l}</span>
          </button>
        ))}
      </div>

      <Sett open={sett} onClose={() => sSett(false)} user={user} onSave={u => sU({ ...user, ...u })} onLogout={() => { sUser(null); DB.s("dia-user", null); }} />
    </div>
  );
}
